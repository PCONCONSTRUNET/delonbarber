-- Update the appointment blocking trigger to block multiple slots based on duration
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    WHILE slot_minutes < duration_minutes LOOP
      ...
      VALUES (NEW.appointment_date, current_slot_time::TEXT, 'Agendamento confirmado', false, NEW.id)
